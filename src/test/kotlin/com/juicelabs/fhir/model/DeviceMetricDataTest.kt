...
class DeviceMetricDataTest : DataTests() {
    @Test
    fun `devicemetric-example 695 Test`() {
        val json = File("./src/test/resources/model/sample_data/devicemetric-example.json").readTextAndClose()
    ...
    @Test
    fun `devicemetric-example 696 Test`() {
        val json = File("./src/test/resources/model/sample_data/devicemetric-example.json").readTextAndClose()
        val obj = mapper.fromJson(json, DeviceMetric::class.java)
        ...
        assertEquals("2016-12-28T09:03:04-05:00", obj.calibration[0].time.toString())
    }
}
