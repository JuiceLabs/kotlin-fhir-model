...
class MessageHeaderDataTest : DataTests() {
    @Test
    fun `messageheader-example 675 Test`() {
        val json = File("./src/test/resources/model/sample_data/messageheader-example.json").readTextAndClose()
    ...
    @Test
    fun `messageheader-example 676 Test`() {
        val json = File("./src/test/resources/model/sample_data/messageheader-example.json").readTextAndClose()
        val obj = mapper.fromJson(json, MessageHeader::class.java)
        ...
        assertEquals("2012-01-04T09:10:14Z", obj.timestamp.toString())
