...
class SearchParameterDataTest : DataTests() {
    @Test
    fun `searchparameter-example-extension 724 Test`() {
        val json = File("./src/test/resources/model/sample_data/searchparameter-example-extension.json").readTextAndClose()
    ...
    @Test
    fun `searchparameter-example-reference 725 Test`() {
        val json = File("./src/test/resources/model/sample_data/searchparameter-example-reference.json").readTextAndClose()
        val obj = mapper.fromJson(json, SearchParameter::class.java)
        assertTrue(stringMatch("SearchParameter", obj.resourceType))
        ...
        assertTrue(stringMatch("draft", obj.status))
        assertEquals(true, if (obj.experimental != null) obj.experimental else false, "Field: obj.experimental")
        assertEquals("2013-10-23", obj.date.toString())
        assertTrue(stringMatch("Health Level Seven Internatio", obj.publisher))
    ...
    @Test
    fun `searchparameter-example 726 Test`() {
        val json = File("./src/test/resources/model/sample_data/searchparameter-example.json").readTextAndClose()
        val obj = mapper.fromJson(json, SearchParameter::class.java)
        assertTrue(stringMatch("SearchParameter", obj.resourceType))
    ...
    @Test
    fun `searchparameter-example 727 Test`() {
        val json = File("./src/test/resources/model/sample_data/searchparameter-example.json").readTextAndClose()
        val obj = mapper.fromJson(json, SearchParameter::class.java)
        assertTrue(stringMatch("url", obj.contact[0].telecom[0].system))
        assertTrue(stringMatch("http://hl7.org/fhir", obj.contact[0].telecom[0].value))
