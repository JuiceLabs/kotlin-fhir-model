...
class AccountDataTest : DataTests() {
    @Test
    fun `account-example 953 Test`() {
        val json = File("./src/test/resources/model/sample_data/account-example.json").readTextAndClose()
        ...
        assertEquals("2016-01-01", obj.period!!.start.toString())
        assertEquals("2016-06-30", obj.period!!.end.toString())
        assertEquals("2016-01-01", obj.active!!.start.toString())
        assertEquals("2016-06-30", obj.active!!.end.toString())
    }

    @Test
    fun `account-example 954 Test`() {
        val json = File("./src/test/resources/model/sample_data/account-example.json").readTextAndClose()
        val obj = mapper.fromJson(json, Account::class.java)
    ...
    @Test
    fun `account-example-with-guarantor 955 Test`() {
        val json = File("./src/test/resources/model/sample_data/account-example-with-guarantor.json").readTextAndClose()
        val obj = mapper.fromJson(json, Account::class.java)
        assertTrue(stringMatch("Account", obj.resourceType))
        ...
        assertEquals("2016-01-01", obj.guarantor[0].period!!.start.toString())
    }
}
