...
class PractitionerRoleDataTest : DataTests() {
    @Test
    fun `practitionerrole-example 988 Test`() {
        val json = File("./src/test/resources/model/sample_data/practitionerrole-example.json").readTextAndClose()
        ...
        assertEquals("2012-01-01", obj.period!!.start.toString())
        assertEquals("2012-03-31", obj.period!!.end.toString())
        ...
        assertEquals("09:00:00", obj.availableTime[0].availableStartTime.toString())
        assertEquals("16:30:00", obj.availableTime[0].availableEndTime.toString())
        assertTrue(stringMatch("thu", obj.availableTime[1].daysOfWeek[0]))
        assertTrue(stringMatch("fri", obj.availableTime[1].daysOfWeek[1]))
        assertEquals("09:00:00", obj.availableTime[1].availableStartTime.toString())
        assertEquals("12:00:00", obj.availableTime[1].availableEndTime.toString())
        assertTrue(stringMatch("Adam will be on extended leav", obj.notAvailable[0].description))
        assertEquals("2017-05-01", obj.notAvailable[0].during!!.start.toString())
        assertEquals("2017-05-20", obj.notAvailable[0].during!!.end.toString())
