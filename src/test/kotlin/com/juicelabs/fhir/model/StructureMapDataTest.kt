...
class StructureMapDataTest : DataTests() {
    @Test
    fun `structuremap-example 995 Test`() {
        val json = File("./src/test/resources/model/sample_data/structuremap-example.json").readTextAndClose()
        ...
        assertEquals("2017-03-09", obj.date.toString())
    ...
    @Test
    fun `structuremap-example 996 Test`() {
        val json = File("./src/test/resources/model/sample_data/structuremap-example.json").readTextAndClose()
        val obj = mapper.fromJson(json, StructureMap::class.java)
