...
class RelatedPersonDataTest : DataTests() {
    @Test
    fun `relatedperson-example-peter 773 Test`() {
        val json = File("./src/test/resources/model/sample_data/relatedperson-example-peter.json").readTextAndClose()
    ...
    @Test
    fun `relatedperson-example-peter 774 Test`() {
        val json = File("./src/test/resources/model/sample_data/relatedperson-example-peter.json").readTextAndClose()
        val obj = mapper.fromJson(json, RelatedPerson::class.java)
        ...
        assertEquals("2012-03-11", obj.period!!.start.toString())
    }

    @Test
    fun `relatedperson-example-f001-sarah 775 Test`() {
        val json = File("./src/test/resources/model/sample_data/relatedperson-example-f001-sarah.json").readTextAndClose()
        val obj = mapper.fromJson(json, RelatedPerson::class.java)
        assertTrue(stringMatch("RelatedPerson", obj.resourceType))
    ...
    @Test
    fun `relatedperson-example 776 Test`() {
        val json = File("./src/test/resources/model/sample_data/relatedperson-example.json").readTextAndClose()
        val obj = mapper.fromJson(json, RelatedPerson::class.java)
        assertTrue(stringMatch("RelatedPerson", obj.resourceType))
    ...
    @Test
    fun `relatedperson-example-f002-ariadne 777 Test`() {
        val json = File("./src/test/resources/model/sample_data/relatedperson-example-f002-ariadne.json").readTextAndClose()
        val obj = mapper.fromJson(json, RelatedPerson::class.java)
        assertTrue(stringMatch("RelatedPerson", obj.resourceType))
    ...
    @Test
    fun `relatedperson-example-f002-ariadne 778 Test`() {
        val json = File("./src/test/resources/model/sample_data/relatedperson-example-f002-ariadne.json").readTextAndClose()
        val obj = mapper.fromJson(json, RelatedPerson::class.java)
        assertTrue(stringMatch("phone", obj.telecom[0].system))
        ...
        assertTrue(stringMatch("female", obj.gender))
        assertEquals("1963", obj.birthDate.toString())
        assertTrue(stringMatch("image/jpeg", obj.photo[0].contentType))
        assertTrue(stringMatch("/9j/4AAQSkZJRgABAQEAlgCWAAD/2", obj.photo[0].data))
        assertEquals("1975", obj.period!!.start.toString())
    }
}
