...
class ProcessResponseDataTest : DataTests() {
    @Test
    fun `processresponse-example 743 Test`() {
        val json = File("./src/test/resources/model/sample_data/processresponse-example.json").readTextAndClose()
    ...
    @Test
    fun `processresponse-example-error 744 Test`() {
        val json = File("./src/test/resources/model/sample_data/processresponse-example-error.json").readTextAndClose()
        val obj = mapper.fromJson(json, ProcessResponse::class.java)
        assertTrue(stringMatch("ProcessResponse", obj.resourceType))
        ...
        assertTrue(stringMatch("active", obj.status))
        assertEquals("2014-07-14", obj.created.toString())
        assertTrue(stringMatch("Organization/2", obj.organization!!.reference))
        assertTrue(stringMatch("http://happyvalley.com/fhir/c", obj.request!!.reference))
        assertTrue(stringMatch("http://hl7.org/fhir/processou", obj.outcome!!.coding[0].system))
    ...
    @Test
    fun `processresponse-example-pended 745 Test`() {
        val json = File("./src/test/resources/model/sample_data/processresponse-example-pended.json").readTextAndClose()
        val obj = mapper.fromJson(json, ProcessResponse::class.java)
        assertTrue(stringMatch("ProcessResponse", obj.resourceType))
    ...
    @Test
    fun `processresponse-example-pended 746 Test`() {
        val json = File("./src/test/resources/model/sample_data/processresponse-example-pended.json").readTextAndClose()
        val obj = mapper.fromJson(json, ProcessResponse::class.java)
        ...
        assertEquals("2016-06-10T11:01:10-08:00", (obj.contained[0] as CommunicationRequest).authoredOn.toString())
